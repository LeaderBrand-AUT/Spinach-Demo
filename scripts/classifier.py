...
def classifyFrame(frame: bytes) -> str:
    try: 
        ...
        model = tf.keras.models.load_model("./scripts/models/" + constants.CURRENT_MODEL)
        ...
        f = open("./scripts/classifications/" + currentDate + "_classification.txt", "x")

        f.write(reportStr)
        f.close()

        # Development, save image along with report
        cv2.imwrite("./scripts/classifications/images/" + currentDate + ".jpg", frame)
